    ...
    *,
    ...
    for a in accounts:
        if include_opening and a.opening_balance is not None:
            expected[a.name] = (1, Decimal(_two(a.opening_balance)))
        else:
    ...
    for txn in transactions:
        for name, sign in _transaction_sides(txn, ours):
            ...
            expected[name] = (count + 1, total + Decimal(_two(delta)))
    return expected


def verify_qif(
    text: str, accounts: list[Account], transactions: list[Transaction]
) -> list[AccountParity]:
    """Compare a rendered QIF document against the pulled data."""
    parsed = parse_qif(text)
    expected = expected_parity(accounts, transactions, include_opening=True)
    parities = []
    for name, (exp_count, exp_sum) in sorted(expected.items()):
        rows = parsed.get(name, [])
        actual_sum = sum((amt for _, amt in rows), Decimal(0))
        parities.append(
            AccountParity(name, exp_count, len(rows), exp_sum, Decimal(_two(actual_sum)))
        )
    return parities


def verify_skg(
    path: Path, accounts: list[Account], transactions: list[Transaction]
) -> list[AccountParity]:
    ...
    actual = expected_parity(accounts, mapper.transactions, include_opening=False)
    # Opening balances live in account.f_importbalance, not as operations.
    expected = expected_parity(accounts, transactions, include_opening=False)
    parities = []
